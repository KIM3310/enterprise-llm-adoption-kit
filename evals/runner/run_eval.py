...
try:
    from app.snowflake_adapter import store_eval_batch
except Exception:  # pragma: no cover - runner works without backend import path
    store_eval_batch = None
...
try:
    ...
    end_eval_run = None
    log_eval_metrics = None
    log_eval_params = None
    start_eval_run = None
    store_eval_run_delta = None
...
def run_eval(dataset_path: Path, base_url: str, baseline_path: Path) -> None:
    ...
    if start_eval_run is not None:
        databricks_run_id = start_eval_run(
            run_name=f"eval-{dataset_path.stem}",
            dataset=str(dataset_path),
            tags={"base_url": base_url},
        )
    if log_eval_params is not None:
        log_eval_params({"dataset": str(dataset_path), "base_url": base_url, "sample_count": len(data)})
    ...
    if store_eval_batch is not None:
        normalized = []
        for item in results:
            sample_scores = item["scores"]
            normalized.append(
                {
                    "dataset": str(dataset_path),
                    "sample_index": int(item["id"]) if str(item["id"]).isdigit() else 0,
                    "use_case": item["use_case"],
                    "role": "Employee",
                    "accuracy": sample_scores["accuracy"],
                    "groundedness": sample_scores["groundedness"],
                    "helpfulness": sample_scores["helpfulness"],
                    "safety": sample_scores["safety"],
                    "latency_ms": 0.0,
        ...
        store_eval_batch(run_id, normalized)
    if log_eval_metrics is not None:
        log_eval_metrics(
            accuracy=summary.get("accuracy", 0.0),
            groundedness=summary.get("groundedness", 0.0),
            helpfulness=summary.get("helpfulness", 0.0),
            safety=summary.get("safety", 0.0),
            avg_latency_ms=0.0,
            total_samples=len(results),
        )
    if store_eval_run_delta is not None:
        store_eval_run_delta(
            run_id=run_id,
            run_name=f"eval-{dataset_path.stem}",
            dataset=str(dataset_path),
            total_samples=len(results),
            avg_accuracy=summary.get("accuracy", 0.0),
            avg_groundedness=summary.get("groundedness", 0.0),
            avg_helpfulness=summary.get("helpfulness", 0.0),
            avg_safety=summary.get("safety", 0.0),
            avg_latency_ms=0.0,
            mlflow_run_id=databricks_run_id or "",
            metadata={"base_url": base_url},
        )
    if end_eval_run is not None:
        end_eval_run("FINISHED")
