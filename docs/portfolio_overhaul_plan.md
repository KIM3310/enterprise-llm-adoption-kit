...
- `/ops/service-brief`, `/ops/summary-pack`, `/ops/review-summary`
- existing application bundle docs under `docs/application/`

The repo already proves runtime + governance depth well. The cleanup focus is the front door, evidence mapping, metadata hygiene, and local verification ergonomics.

## Scope
1. Tighten the user-facing entry path in the root docs.
2. Add a concise evidence map / review guide for recruiter, architect, and operator audiences.
3. Improve proof-surface UX with small, additive service-brief/summary-pack enhancements.
...
## Acceptance criteria
- Reviewers can reach the best evidence paths in under 2 minutes from the README.
- The service brief / summary pack expose clearer role-based review lanes without breaking existing contracts.
...
## Planned passes
1. **Docs / README pass**
   - Add a concise validation data map.
...
2. **Proof surface pass**
   - Add role-oriented review-path metadata to the service brief/summary pack.
