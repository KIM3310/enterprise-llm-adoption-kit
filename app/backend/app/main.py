...
from .databricks_adapter import is_enabled as databricks_enabled
...
async def _call_openai_moderation(api_key: str, payload: str) -> None:
    async with httpx.AsyncClient(timeout=20.0) as client:
        ...
        flagged = response.json().get("results", [{}])[0].get("flagged")
    if flagged:
...
app.add_middleware(
    ...
    allow_origin_regex=r"^https://([a-z0-9-]+\.)?enterprise-llm-kit\.pages\.dev$",
...
def _cors_origin_allowed(origin: str) -> bool:
    if origin in {
        ...
        return True
    return bool(re.match(r"^https://([a-z0-9-]+\.)?enterprise-llm-kit\.pages\.dev$", origin))
...
def ops_platform_proof_board() -> Dict[str, object]:
    runtime = get_llm_runtime_settings()
    return {
        "service": settings.app_name,
        ...
        "platforms": {
            ...
            "snowflake": bool(snowflake_enabled()),
            "databricks": bool(databricks_enabled()),
            "ollama": str(runtime.get("provider", "stub")).strip().lower() == "ollama",
        },
        "links": {
            "service_brief": "/ops/service-brief",
            "resource_pack": "/ops/resource-pack",
...
def ops_runtime_scorecard(user=Depends(get_current_user)) -> Dict[str, object]:
    ...
    return build_ops_runtime_scorecard(
        ...
        auth_mode=settings.auth_mode,
        storage_backend=settings.event_storage_backend,
        integrations_require_auth=bool(settings.integrations_require_auth),
        startup_report=getattr(app.state, "startup_report", None),
        circuit_snapshot=_llm_circuit_snapshot(),
        audit_summary=summarize_log(Path(settings.audit_log_path), max_lines=5000),
        daily_cost_usd=get_daily_cost(),
        alerts=evaluate_ops_alerts(),
        service_events=get_recent_service_events(limit=10),
        recent_decisions=get_recent_control_tower_decisions(limit=10),
    )


@app.get("/ops/runtime/scorecard/schema", tags=["ops"], summary="Ops runtime scorecard schema")
