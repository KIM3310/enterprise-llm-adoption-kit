...
def _load_jwt_secrets(default_secret: str) -> Dict[str, str]:
    ...
    if json_path:
        try:
            ...
            logger.warning("JWT_SECRETS_FILE could not be loaded from %s: %s", json_path, exc)
...
class Settings:
    ...
    def __post_init__(self) -> None:
        ...
        if self.jwt_secret == "dev-secret-change" and _app_env not in ("development", "test"):
            raise RuntimeError(
                "JWT_SECRET must be set to a strong secret in production; "
                "the default 'dev-secret-change' is not allowed outside development/test"
            )
